...
test("resetarea parolei autentifică imediat administratorul",async()=>{const route=await readFile(new URL("../app/api/admin/resetare/route.ts",import.meta.url),"utf8");assert.match(route,/response\.cookies\.set\(ADMIN_COOKIE,createAdminSession\(\)/);assert.match(route,/NextResponse\.redirect\(new URL\("\/admin"/)});
