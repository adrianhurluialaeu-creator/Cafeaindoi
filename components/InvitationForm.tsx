...
export default function InvitationForm(){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){
  ...
  try{const r=await fetch("/api/invitatie",{method:"POST",body:d});const x=await r.json();if(!r.ok)throw new Error(x.error||"Trimiterea a eșuat.");setState("sent");window.gtag?.("event","conversion",{send_to:"AW-18467510680/66mnCKWvgYEdEjiz_-VE",value:1.0,currency:"EUR"});form.reset()}catch(err){setError(err instanceof Error?err.message:"Trimiterea a eșuat.");setState("error")}
 }
