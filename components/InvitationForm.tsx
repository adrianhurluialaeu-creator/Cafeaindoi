...
export default function InvitationForm(){
 ...
 function begin(){if(!started.current){started.current=true;trackAnalytics("invitation_started")}}
 function nextFromAbout(){if(!validate(["invite-prenume","invite-varsta","invite-localitate","invite-tara","invite-despre"]))return;trackAnalytics("invitation_step_completed",{step:1});setError("");setStep(2);requestAnimationFrame(()=>document.getElementById("invite-email")?.focus())}
 function nextFromContact(){if(!validate(["invite-email"]))return;if(enabled&&!selectedSlot){setError("Alege ziua și ora cafelei.");return}trackAnalytics("invitation_step_completed",{step:2});setError("");setStep(3)}
 ...
 return <form ref={formRef} onFocusCapture={begin} onSubmit={submit} encType="multipart/form-data" className="invite-form">
  ...
  <section className={step===3?"invite-step active":"invite-step"} aria-hidden={step!==3}>
   ...
   <LiveMediaCapture onChange={value=>{setMedia(value);if(value)trackAnalytics("invitation_media_ready",{type:value.type.startsWith("video/")?"video":"photo"})}}/>
