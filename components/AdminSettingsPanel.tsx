...
export default function AdminSettingsPanel(){
 ...
 return <div className="settings-layout">
  ...
  <div className="settings-main">
   ...
   {tab==="notifications"?<section className="settings-stack"><section className="settings-card"><header><h2>Reguli de notificare</h2><p>Alege canalele și evenimentele importante.</p></header><Toggle checked={s.notifications.push} onChange={v=>update("notifications","push",v)} label="Notificări push"/><Toggle checked={s.notifications.email} onChange={v=>update("notifications","email",v)} label="Notificări email"/><div className="settings-toggle-grid"><Toggle checked={s.notifications.onInvitation} onChange={v=>update("notifications","onInvitation",v)} label="Invitație nouă"/><Toggle checked={s.notifications.onActivation} onChange={v=>update("notifications","onActivation",v)} label="Cont activat"/><Toggle checked={s.notifications.onMessage} onChange={v=>update("notifications","onMessage",v)} label="Mesaj nou"/><Toggle checked={s.notifications.onMeeting} onChange={v=>update("notifications","onMeeting",v)} label="Cafea propusă"/><Toggle checked={s.notifications.onVideo} onChange={v=>update("notifications","onVideo",v)} label="Apel video"/></div><Save busy={busy==="notifications"} onClick={()=>save("notifications")}/></section><section className="settings-card"><PushSettings/></section></section>:null}
