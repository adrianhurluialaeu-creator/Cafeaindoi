...
export async function POST(req:NextRequest){
 ...
 if(!validateSubscription(input))return NextResponse.json({error:"Abonament invalid."},{status:400});await saveSubscription({...input,createdAt:new Date().toISOString(),recipient:"admin"});return NextResponse.json({ok:true})}
...
export async function DELETE(req:NextRequest){if(!authorized(req))return NextResponse.json({error:"Unauthorized"},{status:401});try{const {endpoint}=await req.json();if(typeof endpoint!=="string")return NextResponse.json({error:"Date invalide."},{status:400});await deleteSubscription(endpoint,{recipient:"admin"});return NextResponse.json({ok:true})}catch{return NextResponse.json({error:"Dezactivarea a eșuat."},{status:503})}}
