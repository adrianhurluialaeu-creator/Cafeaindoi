...
export async function POST(req:Request){try{
 ...
 let r:Response;try{r=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({from,to:[to],subject,text,html})})}catch(emailError){console.error("[invitatie] RESEND_NETWORK_ERROR",emailError);try{await deleteInvitation(saved.id)}catch(cleanupError){console.error("[invitatie] CLEANUP_ERROR",cleanupError)}return NextResponse.json({ok:false,error:"Mesajul nu a putut fi trimis."},{status:502})}if(!r.ok){console.error("[invitatie] RESEND_ERROR",await r.text());try{await deleteInvitation(saved.id)}catch(cleanupError){console.error("[invitatie] CLEANUP_ERROR",cleanupError)}return NextResponse.json({ok:false,error:"Mesajul nu a putut fi trimis."},{status:502})}try{await sendPush("Invitație nouă pentru o cafea",`${prenume} a trimis o invitație${schedule.enabled?" cu o oră propusă":""}.`)}catch(pushError){console.error("[invitatie] PUSH_ERROR",pushError)}return NextResponse.json({ok:true});
 }catch(e){console.error("[invitatie] REQUEST_ERROR",e);return NextResponse.json({ok:false,error:"Serviciul nu este disponibil momentan. Încearcă din nou peste câteva minute."},{status:503})}}
