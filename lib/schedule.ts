...
const SETTING_KEY="coffee_schedule";
...
export async function getSchedule():Promise<Schedule>{
 const {data,error}=await createSupabaseAdmin().from("app_settings").select("value").eq("key",SETTING_KEY).maybeSingle();
 if(error)throw error;
 return data?.value as Schedule||defaults;
}
export async function saveSchedule(value:Schedule){
 const {error}=await createSupabaseAdmin().from("app_settings").upsert({key:SETTING_KEY,value,updated_at:new Date().toISOString()});
 if(error)throw error;
 return value;
}
